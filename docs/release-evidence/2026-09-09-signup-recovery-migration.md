...
**Role:** Evidence
**Source:** Restored unchanged in substance from PR #90
...
The single-migration procedure in `../history/CLOUD-MIGRATION-PROPOSAL.md` was adopted
...
## Boundaries and remaining work at that checkpoint
...
to the public release endpoint returned HTTP 403, so this operation did not
claim a fresh live application fingerprint verification.

The isolated engine was not a full Supabase staging environment. Deployed Auth,
scheduler delivery, real browser cookie transport and simultaneous database
clients still required the controlled acceptance checks documented in
`../history/SIGNUP-RECOVERY-CHECKPOINT.md`.

At that checkpoint, publication and controlled email/browser tests still needed
separate approval. These statements record September 9 status and do not define current work.
