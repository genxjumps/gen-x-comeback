-- Recovery flow: on-demand product access, not a proactive lifecycle email.
--
-- 1) The logical-uniqueness index must allow multiple stalled episodes AND
--    multiple recovery requests per plan version, while every other job type
--    stays exactly one logical job per plan version and job version.
-- 2) The two live functions whose ON CONFLICT clauses infer that partial index
--    are re-created with the new matching predicate and otherwise byte-identical
--    behavior.
-- 3) A minimal service-role-only atomic RPC creates an idempotent recovery job
--    plus its queued event for a matching current lead/plan. No backfill.
...
CREATE UNIQUE INDEX email_jobs_logical_key
  ...
  WHERE job_type NOT IN ('stalled','recovery');
...
BEGIN
  ...
  ON CONFLICT (job_type, plan_version_id, job_version) WHERE job_type NOT IN ('stalled','recovery') DO NOTHING
...
-- Idempotent recovery request. Request-level idempotency comes from the
-- globally unique idempotency_key, so replaying one trusted request id can
-- never create a second job or a second queued event, while a later legitimate
-- request id may create another recovery job for the same current plan version.
--
-- Returns nothing: the public route must never learn whether the email matched.
CREATE OR REPLACE FUNCTION public.request_plan_recovery(
...
BEGIN
  ...
  -- Recovery is independent of plan completion, marketing unsubscribe, and
  -- Plan Ready acceptance. It never cancels, defers, or reprioritizes any
  -- proactive lifecycle job, and it consumes no lifecycle gap or cap.
  INSERT INTO public.email_jobs (
    job_type,
    job_version,
    template_version,
...
REVOKE ALL ON FUNCTION public.request_plan_recovery(text, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.request_plan_recovery(text, text) FROM anon;
REVOKE ALL ON FUNCTION public.request_plan_recovery(text, text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.request_plan_recovery(text, text) TO service_role;