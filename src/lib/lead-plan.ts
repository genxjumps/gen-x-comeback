...
export type StartDayOneResult =
  | { ok: true; startedAt: string; newlyStarted: boolean }
  | { ok: false };
...
export type SaveLeadPlanResult = { firstName: string; plan: Plan; replayed: boolean };
export type RegenerateResult = { ok: true; firstName: string; plan: Plan } | { ok: false };
